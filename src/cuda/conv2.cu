...
int main() {
    try {
        auto h_input = load_binary("../results/predictions/golden_pool1.bin");
        auto h_weights = load_binary("../weights/conv2_weight.bin");
        auto h_biases = load_binary("../weights/conv2_bias.bin");
        auto golden_conv2 = load_binary("../results/predictions/golden_conv2.bin");

        int InC = 6, InH = 14, InW = 14;
        int OutC = 16, K = 5;
        int OutH = InH - K + 1; // 14 - 5 + 1 = 10
        int OutW = InW - K + 1; // 14 - 5 + 1 = 10
        ...
        conv2_kernel<<<numBlocks, threadsPerBlock>>>(d_input, d_weights, d_biases, d_output, 
        ...
        cout << "Verifying CUDA Conv2...\n";
        verify_tensors(h_output, golden_conv2, 1e-4);
